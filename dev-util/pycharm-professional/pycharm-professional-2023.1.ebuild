...
src_prepare() {
	...
	local remove_me=(
		...
		plugins/cwm-plugin/quiche-native/linux-aarch64/libquiche.so
		plugins/python/helpers/pydev/pydevd_attach_to_process/attach_linux_aarch64.so
	    # if musl is not enabled, delete the library
		$(has_version 'sys-libs/musl' || echo plugins/tailwindcss/server/node.napi.musl-IAP67VWK.node)
	)
...
src_install() {
	...
	fperms 755 ${dir}/bin/{format.sh,fsnotifier,inspect.sh,ltedit.sh,pycharm.sh,remote-dev-server.sh,repair,restart.py}
